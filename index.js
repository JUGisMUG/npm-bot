		...
		try {
		  fetch(`https://registry.npmjs.org/${args[0]}`)
			...
			.then(response => {
			  ...
			  let typescript = () => {
			    if (response.versions[latest].types) return 'Yes'
			    if (response.versions[latest].types === undefined) return 'No'
			  }
			  
			  if (keywords === undefined) {
			    keywords = 'None'
			  } else if (keywords !== undefined) keywords = keywords.join(', ')
			  ...
			  .addFields({
			    ...
			    name: 'Dependencies:',
			    value: Object.keys(response.versions[latest].dependencies).join(', ') || 'None'
			  }, {
			    name: 'License:',
			    value: response.versions[latest].license || 'None'
			  }, {
			    name: 'Repository:',
			    value: `[GitHub Repository](${repository})`
			  }, {
			    name: 'Typescript Supoort:',
			    value: typescript()
			  })
